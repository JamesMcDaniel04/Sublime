...
### Task 4.5: Learning transparency + lifecycle (parity table-stakes)

**Source:** `docs/parity/2026-07-11-behavioral-intelligence-parity.md` — the three gaps every market leader ships.

**Steps:**
- [ ] **"What Sublime has learned" view:** a section on /settings (Workspace tab, under the scanning toggle): lists the org's intelligence memories (learnings + suggestions from the hidden org agent) with kind badge, source connection, created date, and per-item DELETE (soft: status 'dismissed' — check agent-memory's status values — so embeddings still dedupe against deleted items and they never resurface). GET/DELETE via a new `/api/intelligence/learnings` route (authenticated, org-scoped; add GET to route-smoke cases).
- [ ] **Per-connection learning opt-out:** `organizations.settings.scanExclusions: string[]` (entries `<plane>:<connectionRef>`); `scanConnection` skips excluded refs; a "Learning" toggle on each MCP-server row + Klavis card + a note on the Nango grid ("managed in Settings"); settings PATCH allowlist extended to accept scanExclusions (validated shape).
- [ ] **Purge on disconnect:** when a connection is deleted (mcp-connections DELETE route, Klavis instance delete, org teardown), remove its scan insight node (`store.deleteNodes([insight:scan:<plane>:<ref>])` via a new `removeConnectionScanFromGraph` in indexer.ts) and hard-delete the org-agent memories whose title/source references that connection (store `sourceRef: '<plane>:<ref>'` in memory metadata at save time in Task 1's persist path — add it — so purge is a precise query, not string matching). Best-effort + logged, mirroring existing teardown patterns.
- [ ] Verify + commit.

### Task 5: Ship + prove the loop

- [ ] Full gate: typecheck, lint, full tests (incl. DB-gated), build; push main; confirm Vercel READY.
- [ ] **Parity gate:** walk `docs/parity/2026-07-11-behavioral-intelligence-parity.md` — every table-stakes row must be ✅ (or explicitly accepted like #12) before the prod push.
