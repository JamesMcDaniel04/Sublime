...
export const POST = withAuthenticatedApi(async (_request, auth) => {
  ...
  const klavisUserId = `${auth.organizationId}:${auth.dbUser.id}`
  ...
  if (authorized.length) {
    await createServersForTenant(`tenant_${auth.organizationId}`, auth.dbUser.id, auth.organizationId, authorized)
  }
