...
/** Refuse a cron cadence with no expression — it would silently never fire. */
export function validateOverrides(overrides: TemplateOverrides): string | null {
  const schedule = overrides.schedule
  if (schedule?.type === 'cron' && !schedule.cron.trim()) return 'A custom schedule needs a cron expression.'
  if (schedule && (schedule.type === 'daily' || schedule.type === 'weekly' || schedule.type === 'once') && !/^\d{2}:\d{2}$/.test(schedule.time)) {
    return 'A daily, weekly, or one-time schedule needs a time in HH:MM.'
  }
  if (schedule?.type === 'once' && !/^\d{4}-\d{2}-\d{2}$/.test(schedule.runAt ?? '')) return 'A one-time schedule needs a date.'
  return null
}
