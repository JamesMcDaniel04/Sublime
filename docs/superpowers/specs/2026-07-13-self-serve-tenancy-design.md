...
**Every sign-up automatically receives its own organization** (the existing JIT
path, enabled for production). There is **no shared/default org** and no
...
### 1. Enable self-serve JIT tenancy (fixes the 403 block)

- Set `AUTH_ALLOW_JIT_PROVISIONING=true` in the production environment
  (Vercel). This is an **ops step** owned by the owner.
- Update `.env.example` to document that this flag is required for self-serve
  sign-up, and what stays blocked without it.
- No behavioral code change: the JIT path already
...
The flag stays in place (not removed, default not flipped in code): unknown
identities minting workspaces remains an explicit deployment choice, per the
security rationale in the `20260711120000_disable_unsafe_auth_trigger`
migration.
...
- **Flag unset in production** → current fail-closed behavior (403) remains;
...
- Enabling JIT means anyone who can complete sign-up gets a workspace. Email
  confirmation remains per Supabase project settings; the owner can disable
  self-serve at any time by unsetting the env flag.
...
- **Unit — provisioning:** JIT path creates org + ADMIN user when the flag is
  set and no invitation exists; invitation path still wins when present.
