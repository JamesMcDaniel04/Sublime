...
- **Entitlement onboarding (People.ai per-user OAuth connect flow):** delete `src/lib/peopleai/oauth.ts`, `src/lib/peopleai/connect-service.ts`, `src/app/api/peopleai/connect/route.ts`, `src/app/api/peopleai/callback/route.ts`, `src/app/api/peopleai/status/route.ts` (+ their tests). RAG/signals do **not** depend on per-user OAuth — they use the *service* client + webhook receiver.
...
- `src/lib/peopleai/client.ts` (service + read client factories), `src/lib/peopleai/salesai-facts.ts`, `src/lib/peopleai/webhook-secret.ts`, `src/lib/peopleai/register-webhook.ts`.
- Signal receiver: `src/app/api/signals/people-ai/route.ts`, `src/lib/signals/map.ts`, `src/lib/signals/verify.ts`, the generic `signals/*` + `signals/custom/*`.
