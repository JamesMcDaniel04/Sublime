...
**The store.** `StoreListing` is a versioned snapshot of an agent (`src/lib/store/listing.ts`) — native (job, integrations, grants, goal, output contract) or external (the publisher's endpoint contract) — published by a workspace to its members or to every workspace (admin only). It deliberately has no `organizationId`: public listings are cross-workspace, so the model sits outside the tenant guard and every query scopes by visibility. A definition never carries a secret or a workspace-local reference (skills, HTTP endpoints); the publish route asserts that. `AgentInstall` records the version a workspace took and the agent it became, so `POST /api/store/[id]/install` can offer an update — refused over local edits unless forced. A native install goes through `materializeAgent` (extracted from the provision route to `src/lib/templates/materialize-agent.ts` so the store is not a fourth agent-creation path); an external install creates an external agent bound to the publisher's endpoint with the installer's own credential.

**Template customization.** A template is a starting point: the detail page (`/templates/[id]`) has an edit mode for the name, description, instructions, model, and schedule, and a deploy sends only the changed fields as `overrides` on `POST /api/templates/provision`. The route still re-reads the trusted recipe (seed or community row) and applies the overrides onto it (`src/lib/templates/overrides.ts`, strict schema — never the graph, tool bindings, or required integrations); a schedule override is also written onto the flow graph's trigger node (`rewriteGraphTrigger`) so the builder's next save cannot revert it. A multi-agent flow seed keeps its per-step agent instructions and reports `ignoredOverrides`. "Save as my template" forks the customized recipe into an org-owned `AgentTemplate` (the existing idempotent POST), after which it is editable in place with PUT; "Save changes" does that for templates the org already owns. The fields deployed with an override are recorded as `customizedFields` on the agent metadata / flow metadata.

## Workspace Files

The file repository (`/knowledge`) is the same durable substrate as retained knowledge — `KnowledgeDocument` + encrypted, embedded chunks — restricted to the documents people put there: `sourceType` `upload` (a file, extracted to text) or `manual` (a Markdown note written in the app). `POST /api/knowledge` accepts either a multipart file (the shared upload path in `src/lib/knowledge/upload.ts`: size cap, malware scan, signature check, extraction — the per-agent upload route uses the same function) or a JSON note; `GET/PUT /api/knowledge/[id]` read and edit a file in place (the edit re-chunks and re-embeds under the same id, keyed by `sourceId = id`). Visibility is the retrieval rule: `organization` for every agent, `private` for the author's own runs, `agent` for files attached from an agent's settings.

Agents reach these files two ways. Retrieval (`retrieveKnowledge`) still injects the most similar passages into the prompt. For "follow onboarding.md in full", a run is additionally offered `list_workspace_files` and `read_workspace_file` (`src/lib/knowledge/file-tools.ts`, provider `knowledge`, read-only so never approval-gated, withheld when the agent's grant blocks that plane, and not built at all when the run can read no files). Reads are paged and fenced as untrusted content like every other retrieved passage. Auto-captured knowledge (run summaries, connection profiles, activity) is deliberately outside this surface: it is background context, not a file someone maintains.
