...
test('isWriteProvider classifies delivery planes as writes and reads as reads', () => {
  ...
  assert.equal(isWriteProvider('den'), false) // People.ai read plane
  assert.equal(isWriteProvider('granola'), false)
  assert.equal(isWriteProvider('github'), false) // unknown/Klavis read
})

test('every write connector is a delivery plane; den is read', () => {
  const den = BUILTIN_CONNECTORS.find((c) => c.providerId === 'den')!
  assert.equal(den.isWrite, false)
