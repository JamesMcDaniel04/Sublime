...
export type GoalTemplate = {
  ...
  /** Only meaningful for `kpi` (arr and quota derive usd from the kind). */
...
const template = (
  ...
  description,
  kind,
  // The kind no longer implies direction: 'savings' collapsed into 'kpi'
  // (spec 2026-07-28), so cost-reduction templates state `direction` outright.
  direction: spec.direction ?? 'increase',
  // 'kpi' has no implied unit, so a kpi template that wants usd must say so —
  // the 'count' fallback is only right for counting goals.
  unit: spec.unit ?? GOAL_KIND_UNITS[kind] ?? 'count',
  // Only quota still implies a cadence. The former mrr/lead_gen monthly
  // inference died with those kinds; those templates state it explicitly.
  recurrence:
...
export const GOAL_TEMPLATES: GoalTemplate[] = [
  // ── Sales ────────────────────────────────────────────────────────────────
  template('sales-org-quarterly-revenue', 'sales', 'org', 'Quarterly revenue target', 'Track closed-won revenue against the number the team committed to.', 'arr', {
  ...
  template('sales-personal-monthly-closed', 'sales', 'personal', 'Close more revenue this month', 'A personal monthly closed-won target that resets every cycle.', 'arr', {
  ...
  template('sales-personal-revive-stalled-deals', 'sales', 'personal', 'Revive every stalled deal', 'Deals with no touch in two weeks get a grounded re-entry, not a nudge.', 'kpi', {
  ...
  template('sales-personal-followup-same-day', 'sales', 'personal', 'Follow up before the day ends', 'Every meeting turns into a follow-up that repeats what was actually committed.', 'kpi', {
    motion: 'action', category: 'Pipeline', unit: 'count', recurrence: 'monthly', layout: PERSONAL_LAYOUT,
  ...
  // ── Marketing ───────────────────────────────────────────────────────────
  template('marketing-org-monthly-mqls', 'marketing', 'org', 'Monthly qualified leads', 'Lead generation against a monthly MQL target from your CRM or spreadsheet.', 'kpi', {
    recurrence: 'monthly', motion: 'outcome', category: 'Demand', layout: COUNT_LAYOUT,
  ...
  template('marketing-org-inbound-mrr', 'marketing', 'org', 'Grow inbound-sourced MRR', 'Recurring revenue attributed to marketing-sourced pipeline.', 'arr', {
    recurrence: 'monthly', motion: 'outcome', category: 'Revenue', layout: REVENUE_LAYOUT,
  ...
  template('marketing-personal-campaign-leads', 'marketing', 'personal', 'Leads from my campaigns', 'A personal lead target for the campaigns you own this month.', 'kpi', {
    recurrence: 'monthly', motion: 'outcome', category: 'Demand', layout: PERSONAL_LAYOUT,
    tracks: 'Leads attributed to campaigns you own, this month.',
    sources: ['hubspot', 'salesforce', 'google_sheets'],
    agents: ['mkt-inbound-mql-router', 'marketing-campaign-command-center'],
  }),
  template('marketing-personal-newsletter', 'marketing', 'personal', 'Grow newsletter signups', 'Signups tracked from your list tool, a sheet, or a dashboard URL.', 'kpi', {
  ...
  template('marketing-org-organic-traffic', 'marketing', 'org', 'Grow organic traffic', 'Non-paid sessions, the compounding channel.', 'kpi', {
  ...
  template('marketing-personal-repurpose-every-piece', 'marketing', 'personal', 'Repurpose every piece I publish', 'One piece becomes every channel it should have been on.', 'kpi', {
  ...
  template('marketing-personal-messaging-from-customers', 'marketing', 'personal', 'Ground my messaging in customer words', 'Claims trace back to something a customer actually said.', 'kpi', {
    motion: 'action', category: 'Demand', unit: 'count', recurrence: 'monthly', layout: PERSONAL_LAYOUT,
  ...
  // ── Engineering ─────────────────────────────────────────────────────────
  template('engineering-org-infra-savings', 'engineering', 'org', 'Cut infrastructure spend', 'A cost-reduction target — the trendline should go DOWN.', 'kpi', {
    direction: 'decrease', unit: 'usd', motion: 'outcome', category: 'Cost', recurrence: 'quarterly', layout: COST_LAYOUT,
    tracks: 'Monthly cloud and infrastructure spend, trending toward your floor.',
    sources: ['postgres', 'google_sheets', 'url'],
    agents: [],
  }),
  template('engineering-org-open-bugs', 'engineering', 'org', 'Reduce open bug count', 'Drive the open-defect count down and keep it down.', 'kpi', {
  ...
  template('engineering-personal-bug-backlog', 'engineering', 'personal', 'Clear my bug backlog', 'Your personally-assigned open issues, trending to a target.', 'kpi', {
  ...
  template('engineering-personal-ship-cadence', 'engineering', 'personal', 'Ship N releases this quarter', 'A personal shipping-cadence target for the quarter.', 'kpi', {
  ...
  template('engineering-org-release-go-no-go', 'engineering', 'org', 'A go/no-go brief before every release', 'Every release gets a named decision instead of a hopeful deploy.', 'kpi', {
    motion: 'action', category: 'Delivery', unit: 'count', recurrence: 'monthly', layout: COUNT_LAYOUT,
  ...
  template('engineering-org-p1-incidents', 'engineering', 'org', 'Cut Sev-1 incidents', 'The count of the incidents that wake people up.', 'kpi', {
  ...
  template('engineering-org-incident-context-packet', 'engineering', 'org', 'A context packet on every incident', 'Responders open a timeline, not a blank channel.', 'kpi', {
  ...
  template('engineering-personal-review-turnaround', 'engineering', 'personal', 'Review PRs faster', 'Hours between a review being requested of you and you giving it.', 'kpi', {
  ...
  template('engineering-personal-capture-every-decision', 'engineering', 'personal', 'Capture every architecture decision', 'Decisions made in review threads become findable records.', 'kpi', {
  ...
  // ── Finance ─────────────────────────────────────────────────────────────
  template('finance-org-vendor-savings', 'finance', 'org', 'Reduce vendor spend', 'A company savings target across renegotiated and retired vendors.', 'kpi', {
    direction: 'decrease', unit: 'usd', motion: 'outcome', category: 'Cost', layout: COST_LAYOUT,
  ...
  template('finance-org-collected-revenue', 'finance', 'org', 'Collected-revenue target', 'Cash actually collected, not just booked — tracked from your ledger.', 'arr', {
    motion: 'outcome', category: 'Revenue', recurrence: 'quarterly', layout: ORG_ROLLUP_LAYOUT,
  ...
  template('finance-personal-cost-center', 'finance', 'personal', 'Cut my cost center spend', 'A personal savings target for the budget lines you own.', 'kpi', {
    direction: 'decrease', unit: 'usd', motion: 'outcome', category: 'Cost', layout: PERSONAL_LAYOUT,
    tracks: 'Spend on the budget lines you own.',
    sources: ['postgres', 'google_sheets'],
    agents: ['finance-spend-exception-review'],
  }),
  template('finance-personal-dso', 'finance', 'personal', 'Bring DSO down', 'Days sales outstanding, trending down toward a target.', 'kpi', {
  ...
  template('finance-org-gross-margin', 'finance', 'org', 'Improve gross margin', 'Revenue less cost of revenue, as a percentage.', 'kpi', {
  ...
  template('finance-org-work-every-overdue-invoice', 'finance', 'org', 'Work every overdue invoice', 'Collections get worked by value and relationship, not by spreadsheet order.', 'kpi', {
  ...
  template('finance-org-review-every-spend-exception', 'finance', 'org', 'Review every spend exception', 'Unusual spend gets a named reviewer before it becomes a surprise.', 'kpi', {
    motion: 'action', category: 'Cost', unit: 'count', recurrence: 'monthly', layout: COUNT_LAYOUT,
  ...
  template('finance-personal-close-cycle', 'finance', 'personal', 'Close the books faster', 'Business days from period end to a closed set of books.', 'kpi', {
  ...
  template('finance-personal-explain-every-variance', 'finance', 'personal', 'Explain every material variance', 'Variances arrive with a reason attached, not a question mark.', 'kpi', {
  ...
  // ── CSM ─────────────────────────────────────────────────────────────────
  template('csm-org-nrr', 'csm', 'org', 'Net revenue retention', 'NRR as a percentage target — the health metric of the book.', 'kpi', {
  ...
  template('csm-org-expansion-mrr', 'csm', 'org', 'Grow expansion MRR', 'Upsell and expansion recurring revenue, tracked monthly.', 'arr', {
    recurrence: 'monthly', motion: 'outcome', category: 'Revenue', layout: REVENUE_LAYOUT,
  ...
  template('csm-personal-renewals', 'csm', 'personal', 'Renewals closed this quarter', 'Your renewal revenue against the quarter’s book.', 'arr', {
  ...
  template('csm-personal-churn-saves', 'csm', 'personal', 'Reduce churned accounts', 'Accounts lost from your book, trending down.', 'kpi', {
  ...
  template('csm-org-gross-retention', 'csm', 'org', 'Gross revenue retention', 'Retention before any expansion — the honest churn number.', 'kpi', {
    motion: 'outcome', category: 'Retention', unit: 'percent', layout: RATE_LAYOUT,
  ...
  template('csm-personal-brief-every-qbr', 'csm', 'personal', 'A real brief before every QBR', 'You walk in with outcomes and an ask, not a slide of usage charts.', 'kpi', {
  ...
  template('csm-personal-work-every-risk-flag', 'csm', 'personal', 'Work every risk flag in my book', 'A risk signal becomes a save play the same week it appears.', 'kpi', {
    motion: 'action', category: 'Retention', unit: 'count', recurrence: 'monthly', layout: PERSONAL_LAYOUT,
