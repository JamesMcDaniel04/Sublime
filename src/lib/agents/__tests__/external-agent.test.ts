...
import {
  ...
  interpretCallbackBody, interpretExternalResponse, mintCallbackToken, verifyCallbackToken,
} from '../external-agent'
...
test('200 with output answers inline; 202 accepts; anything else fails with the status', () => {
  assert.deepEqual(interpretExternalResponse(200, { output: 'hi' }), { kind: 'completed', output: 'hi' })
  assert.deepEqual(interpretExternalResponse(200, { output: { a: 1 } }), { kind: 'completed', output: '{"a":1}' })
...
test('the callback body follows the same rules', () => {
  assert.deepEqual(interpretCallbackBody({ output: 'done' }), { kind: 'completed', output: 'done' })
...
test('the payload names its protocol and carries the ask verbatim', () => {
  const payload = buildExternalPayload({ runId: 'r', agentId: 'a', request: { id: 'q', text: 'hello', requesterName: 'Jamie' }, objective: 'o', input: 'hello', callbackUrl: 'u', callbackToken: 't' })
