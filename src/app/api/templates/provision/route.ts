...
async function loadDbTemplateRecipe(templateId: string, organizationId: string): Promise<DbTemplateRecipe> {
  ...
  // The SAME rule the listing uses (lib/templates/visibility.ts), because this
  // path matters more: listing another workspace's private template discloses
  // it, provisioning it copies their process into your workspace. Covers both
  // auto-generated process intelligence and the explicit org/community setting
  // a template saved from a flow carries. A 404 rather than a 403 — a template
  // you may not read should not confirm its own existence.
  if (!row || !templateVisibleTo(config, row.organizationId, organizationId)) {
