...
export const graphRules =
  ...
  'Per-item fan-out: http/tool/transform/data steps accept forEachItem: true — the step runs once per item of its input list ({{item.…}} refs resolve per iteration, capped at 500 items) and outputs the collected array. ' +
  'Item routing: filter with splitItems: true keeps the MATCHING items of the input list and always continues (empty list flows on); condition with splitItems: true splits items into {matched, unmatched} on its output, runs BOTH branches when both sides are non-empty, and downstream steps read {{step.<conditionId>.output.matched}} / .unmatched. ' +
  'Inline JS expressions: any templated field may embed {{js: <expression>}} — evaluated server-side in a sandbox with $json (the current item or trigger input), step (step outputs by id, e.g. step.fetch.body), input, trigger, vars, item, and loop in scope. Use it for quick inline computation (string methods, arithmetic, ternaries, mapping) instead of adding a Code step; e.g. {{js: input.name.toUpperCase()}} or {{js: step.api.body.total * 1.1}}. An exact whole-field {{js: …}} token keeps the expression\'s native type (number/array/object). Object-literal expressions with { } are not supported inline (use a Code step). ' +
  'Polling trigger: trigger data {type:"poll", intervalMinutes?, source:{connectionId, toolName, args?, itemsPath?, idPath?}} — the platform runs the READ action on the interval (min 5 minutes), diffs results by idPath (default "id"), and starts ONE run per NEW item with the item as {{trigger.input}}. Use it for "when a new row/record/message appears" flows; the first poll baselines silently.'
