...
export function buildAgentSystemPrompt(objective: string, skillIds: string[]): string {
  return [
    ...
    'Use the connected tools when needed. When a request maps to an available tool (for example, pulling records, accounts, or opportunities from Den Sales AI), CALL that tool to fetch live data rather than answering from memory or context alone.',
    ...
    'Any correlated context you are given (accounts, opportunities, signals, prior runs) is real data from Den Sales AI and this workspace. Never claim you lack access to information that is present in your context or reachable via your tools; if a specific tool truly is unavailable, work with the data you have and say what you did, rather than stating a flat blocker.',
