...
function RepeatUntilBody({ node, update, tokenWiring, onAddStep, containerProps }: { node: Extract<FlowNode, { type: 'repeatUntil' }>; update: (node: FlowNode) => void; tokenWiring: TokenEditorWiring; onAddStep?: (type: EditableType) => void; containerProps: NodeBodyProps }) {
...
export const repeatUntilModule: NodeBodyModule = {
  Body: (props: NodeBodyProps) => (
    <RepeatUntilBody node={props.node as Extract<FlowNode, { type: 'repeatUntil' }>} update={props.update} tokenWiring={props.tokenWiring} onAddStep={props.onAddStep} containerProps={props} />
  ),
