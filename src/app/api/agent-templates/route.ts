...
function serializeTemplate(template: any, viewerOrgId?: string) {
  ...
  return {
    // Resolved ONCE here, by the shared rule, so the ordering filter below
    // never re-decides it. Covers autoGenerated (org process intelligence) and
    // the explicit org/community visibility a saved flow template carries.
    visibleToViewer: templateVisibleTo(config, template.organizationId, viewerOrgId ?? null),
    id: template.id,
    name: template.name,
...
export const GET = withAuthenticatedApi(async (request, auth) => {
  ...
  const builtIns = builtInTemplates.map((t) => ({
    ...
    mine: false,
    autoGenerated: false,
    // Built-ins ship with the app — there is no owning org to scope them to.
