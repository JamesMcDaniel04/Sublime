...
- **`agentId:'org:shared'` synthetic id** — verify `agent_memories.agentId` has no FK constraint before Task 1 lands; if it does, fall back to a real hidden AgentTask row per org.
- **Vercel fire-and-forget** — `after()` required for reliability; verify Next version first.
