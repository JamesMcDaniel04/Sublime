...
const DIFFERENTLY_AUTHENTICATED: ReadonlyArray<{ route: string; mechanism: string }> = [
  ...
  { route: 'mcp', mechanism: 'workspace API key + flows:execute OR agents:execute scope at the door; each tool re-checks its own scope (flows:execute for flow tools, agents:execute for ask_agent, agents:read for get_request)' },
  ...
  { route: 'flows/[id]/form', mechanism: 'per-flow trigger token in the query string — the caller is a browser and cannot set a header; flow must be ACTIVE and trigger.type must be form, and the endpoint is per-flow + per-IP throttled' },
  ...
  { route: 'agents/[id]/external/callback', mechanism: 'single-use, run-bound callback token minted at dispatch to an external agent; the settle write clears it, so replay/late/cancelled callbacks all 401' },
