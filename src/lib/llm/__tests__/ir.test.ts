...
const anthropicMessage = {
  content: [
    ...
    { type: 'tool_use', id: 'toolu_1', name: 'den_get_account', input: { account: 'ACME' } },
  ],
...
test('irFromAnthropic keeps neutral fields AND raw native content', () => {
  ...
  assert.deepEqual(ir.toolCalls, [{ id: 'toolu_1', name: 'den_get_account', input: { account: 'ACME' } }])
...
test('cross-provider translation DROPS thinking, keeps text + tool calls', () => {
  ...
  assert.equal(assistant.tool_calls?.[0].function.name, 'den_get_account')
